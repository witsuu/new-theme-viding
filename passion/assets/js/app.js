...
document.querySelector("#btn-envelope").addEventListener("click", () => {
	...
	document.querySelector(".envelope-wrap").style.transform = "translateY(-110%)"
...
if (gallery) {
	const gridLg = [0, 7, 8, 14];
	const gridSm = [0, 5, 6, 11, 12];
	...
	const switchGrid = (mqh) => {
		...
		galleryItems.forEach((el, index) => {
			if (mqh.matches) {
				gridSm.includes(index) && el.classList.add("grid-lg-default");
			} else {
				gridLg.includes(index) && el.classList.add("grid-lg-default");
			}
		});
	};
